 ...
 */
export default class GitRepository2 {
	 ...
	 */
	static reset(): void {
		GitRepository2.gitModels = new Map();
		GitRepository2.simplegits = new Map();
	}
	...
	private static getSimplegit(repositoryPath: string = ''): any {
		if (GitRepository2.simplegits.has(repositoryPath)) {
			return GitRepository2.simplegits.get(repositoryPath);
		}
		const simplegit = require('simple-git/promise')(join(getWorkspacePath(), repositoryPath));
		GitRepository2.simplegits.set(repositoryPath, simplegit);
	...
	static async getGitModel(repositoryPath: string = '', mainRepositoryPath: string = ''): Promise<Git> {
		...
		let gitModel = GitRepository2.gitModels.get(repositoryPath);
		if (gitModel) {
			return gitModel;
		}

		// no Git-Model was created yet => create a new one
		gitModel = await GitRepository2.updateGitModel(repositoryPath, mainRepositoryPath);
	...
	private static setGitModel(repositoryPath: string = '', gitModel: Git): void {
		repositoryPath = repositoryPath.replace(/\\/gi, '/');
		const oldModel = GitRepository2.gitModels.get(repositoryPath);
		GitRepository2.gitModels.set(repositoryPath, gitModel);
	...
	static async isUnimportantGitFile(filename: string): Promise<boolean> {
		...
		const subModuleRoots = (await GitRepository2.getGitModel())
			.getSubmodules()
	...
	static async isFileInSubmodule(filename: string): Promise<string> {
		const submodule = (await GitRepository2.getGitModel())
			.getSubmodules()
	...
	static isCurrentlyUpdating(): boolean {
		return GitRepository2.isUpdating;
	}
	...
	private static updatingStart(): void {
		GitRepository2.isUpdating = true;
	}
	...
	private static updatingEnd(repositoryPath: string, branch?: string): void {
		GitRepository2.updateGUI(repositoryPath, branch);
		GitRepository2.isUpdating = false;
	}
	...
	static async updateGUI(repositoryPath: string, branch?: string): Promise<void> {
		...
		const simplegit = GitRepository2.getSimplegit(repositoryPath);

		const oldBranch = branch ? branch : (await GitRepository2.getGitModel(repositoryPath)).getBranch();
	...
	static async updateGitModel(repositoryPath: string = '', mainRepositoryPath: string = ''): Promise<Git> {
		let simplegit;
		try {
			simplegit = GitRepository2.getSimplegit(repositoryPath);
		} catch (error) {
		...
		GitRepository2.setGitModel(repositoryPath, gitModel);
	...
	static async getModelDiff(repositoryPath: string = ''): Promise<any> {
		const newModel = await GitRepository2.getGitModel(repositoryPath);
	...
	/* INIT */
	/*******************************************************************************************/
	static async checkIfisGit(folderPath: string): Promise<boolean> {
		let bb = folderPath + '/.git';
		try {

			await fs.promises.access(bb);
			return true;
		}
		catch (error) {
	...
	static async init(): Promise<void> {
		if (await GitRepository2.checkIfisGit(getWorkspacePath())) {
			GitRepository2.updateGitModel2(getWorkspacePath());
		}
		else {
			let inputFolder = getWorkspacePath();
			await fs.promises.readdir(inputFolder)
				.then(async fileNames => {
					for (let fileName of fileNames) {
						let dirContent = path.resolve(inputFolder, fileName);
						if (fs.statSync(dirContent).isDirectory()) {
							if (await GitRepository2.checkIfisGit(dirContent)) {
								GitRepository2.updateGitModel2(dirContent);
							}
						}
					}
				})
		...
		GitRepository2.gitModels.forEach((value: Git, key: string, map: Map<string, Git>) => {
			Logger.showMessage(key + "----" + value);
			console.log(key + "----" + value);
		});

		return;
		//const gitInstance = GitRepository2.getSimplegit();
		//await gitInstance.init();
		//await gitInstance.add('./*');
		//await gitInstance.commit('first commit');
	}
	static updateGitModel2(repositoryPath: string): void {
		const gitModel = new Git(repositoryPath);
		GitRepository2.setGitModel(repositoryPath, gitModel);
	}
	...
	static async publishBranch(repositoryPath: string, remote: string, branch: string): Promise<void> {
		...
		GitRepository2.updatingStart();

		await GitRepository2.getSimplegit(repositoryPath).push(remote, branch, { '-u': null }).catch();

		GitRepository2.updatingEnd(repositoryPath);
	...
	static async createNewBranch(repositoryPath: string = '', branch: string): Promise<void> {
		await GitRepository2.getSimplegit(repositoryPath).checkoutLocalBranch(branch);
	...
	static async checkoutBranchForRepository(repositoryPath: string, branch: string): Promise<void> {
		...
		await GitRepository2.getSimplegit(repositoryPath)
			.checkout(branch)
			.catch(() => {
				...
				GitRepository2.updatingEnd(repositoryPath, branch);
	...
	static async getConfiguredBranchForSubmodule(gitModel: Git): Promise<string> {
		return new Promise((resolve) => {
			...
			readFile(join(mainRepsoitoryPath, '/.gitmodules'), 'utf8', (err: any, data: string) => {
				if (!data) {
					return resolve('');
				}
	...
	static async pushRootRepository(): Promise<any> {
		const gitModel = await GitRepository2.getGitModel();

		return GitRepository2.pushRepository(
	...
	static async pullAndPushRepository(
		...
		await GitRepository2.pullRepository(repositoryPath, remote, branch, behind, true).catch((error: any) =>
			GitRepository2.catchPushAndPullRepositoryError(error, repositoryPath, remote, branch, status),
		);

		await GitRepository2.pushRepository(repositoryPath, remote, branch, ahead, true).catch((error: any) =>
			GitRepository2.catchPushAndPullRepositoryError(error, repositoryPath, remote, branch, status),
		);
